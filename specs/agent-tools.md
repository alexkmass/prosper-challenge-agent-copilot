...
- **FR-9**: Creating a contact that was never explicitly created mid-call is **also** available as a
  ...
  `bot.py`'s `on_client_disconnected` inspects `call_store.get(active_id)["state"]` (the same
  accumulator the Call Log panel reads, see [voice-agent-builder-ui.md](voice-agent-builder-ui.md)); if
